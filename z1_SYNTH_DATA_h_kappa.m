...
% % %     isCrust = trumodel.z < trumodel.zmoh; 
% % %     xi = trumodel.Sanis(isCrust)/100+1; % Radial S anisotropy. add one because for some reason here anisotropy is + or -
% % %     xi = (1/mean(1./xi) + mean(xi)) / 2; % Voigt-Reuss-Hill average
% % %     phi = trumodel.Panis(isCrust)/100+1; % Radial P anisotropy
% % %     phi = (1/mean(1./phi) + mean(phi)) / 2; 
% % %     rho = mean(trumodel.rho(isCrust)); % Medium density of crust. Not sure how best to average this yet. 
...
% % % 
% % %     [velVs,~] = christof_radial_anis(trumodel.vsAvCrust, trumodel.vsAvCrust * trumodel.vpvs,...
% % %         xi, phi, eta, rho, incAngVs); % Velocities for ray with vs incidence angle
% % %     [velVp,~] = christof_radial_anis(trumodel.vsAvCrust, trumodel.vsAvCrust * trumodel.vpvs,...
% % %         xi, phi, eta, rho, incAngVs); % Velocities for ray with vp incidence angle
...
% % %         'vpvsANIS_vpvs',vpvsEffective/trumodel.vpvs,'Vsv_av',vsvAn); 
% % % 
% % % % %     [HK_A, HK_H, HK_K] = HKstack(trudata.RF_Ps.PSV(:,2), trudata.RF_Ps.tt, ...
% % % % %         trudata.RF_Ps.rayp/111.1949, phase_wts, ...
% % % % %         vsAv, ...
...
% % %     HK_H = HK_H'; 
% % %     HK_K = HK_K'; 
    [HK_A, HK_H, HK_K] = HKstack_anis_wrapper(par, trumodel, trudata, ...
        'ifplot', true); 
