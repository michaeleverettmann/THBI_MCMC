...
%     toplt = (parm_all > cutoff) | (parm_all < (1-cutoff)); 
    toplt = logical(ones(size(parm_simp))); 
    m_scatter(lon( toplt), lat( toplt), 25, parm_simp( toplt), 'filled'); 
...
%     clim([0, 1]); 
    ...
    % Title 
    m_text(-73.7, 31.5, titles(ifig), 'Units','data', 'HorizontalAlignment', 'center',...
