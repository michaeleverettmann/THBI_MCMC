function [ predata,SW ] = b3_FORWARD_MODEL_SW_precise( model,par,predata,ID )
% [ predata,SW ] = b3_FORWARD_MODEL_SW_precise( model,par,predata,ID )
% 
...
if any(strcmp(allpdytp(:,2),'Lov'))
    ...
    [SW.Lov.phV,SW.Lov.grV,SW.Lov.eigfiles] = run_mineos(...
        model,predata.(itp{1}).for_mod_info.periods_calc,par_mineos,0,0,par.inv.verbose); %MINEOS_REPLACE
end
