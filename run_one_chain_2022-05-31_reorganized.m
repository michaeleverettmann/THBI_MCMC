...
%% Pay attention to the speed of the inversion. 
...
if rem(ii,1*par.inv.saveperN)==0 || ii==2, fprintf(...
    ...
    avgTime, timeStartIter(ii)/ii); 
end
       
%% Tons of stuff to deal with broken, slow, or stuck chains. 
%%% For debugging. If there's an error, save the model and quit. 
...
%%% Reset to Kbase.model or KbasePrev.model if current model causes code to fail alot. 
...
%         warning([failInfoStr 'Error within first 10 iterations. Reseting chain.']); 
    ...
    if resetData; 
        try; 
            ...
            nchain = 0;
            fail_chain = 0; % I think. brb2022.05.31
        catch 
...
% % %     newK = false; 
% % %     resetK = false;
% % %     if fail_chain>19 %brb2022.05.31 Need to figure out a new strategy so that we exit if we already rewound through Kbase and Kbase prev and are still having code break. 
...
if ( (ii-1)>stuckIfNoChange+1) 
    ...
    if all( ~accptArr((ii-1)-stuckIfNoChange:(ii-1)-1) ); % last stuckIfNoChange have been rejected. Somethings wrong! 
        fprintf(['\nbrb2022.04.06 Stuck! Didnt accept model for %1.0f iterations.\n  ',...
                 ...
                 'fail_chain=%1.0f'],...
                 stuckIfNoChange,(ii-1),log_likelihood,log_likelihood1,fail_chain)
        fprintf('Increasing nchain artificially IF fail_chain == 0\nnchain: %1.0f',nchain); 
        if fail_chain == 0; % TODO why did I only want to do this if there wasnt a failure? 
            nchain = nchain + 50; 
        end
...
%     if round_level(temp,0.01)>1%         if par.inv.verbose, fprintf('TEMPERATURE = %.2f\n',temp); end%     end
...
% % %     end
    ...
    if resetK
        try
            ...
            fprintf('Kernel reset BROKE at %s-%.0f... immediately REWIND to %s-%.0f <<<<<<<<<<<<<<< \n',chainstr,ii,chainstr,Kbase.itersave)
            model = Kbase.modelk;
            ii = Kbase.itersave;
