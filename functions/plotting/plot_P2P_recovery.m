function plot_P2P_recovery(prior,posterior,TRUEmodel,par,ifsave,ofile)
...
if nargin<6 || isempty(ofile)
    ofile = 'figs/synth_recovery_p2p_fig.pdf';
end
...
if ~isfield(prior,'Npass')
    ...
    prior.Npass = prior.Nstored;
    prior.fdVSsed = 200*(prior.VScrusttop-prior.VSsedbot)./(prior.VScrusttop+prior.VSsedbot);
    prior.fdVSmoh = 200*(prior.VSmanttop-prior.VScrustbot)./(prior.VSmanttop+prior.VScrustbot);
end
...
zsed_tru = tm.zsed;
zmoh_tru = tm.zmoh;
vss1_tru = tm.VS(tm.Z==0);
vss2_tru = min(tm.VS(tm.Z==zsed_tru));
vsc1_tru = max(tm.VS(tm.Z==zsed_tru));
vsc2_tru = min(tm.VS(tm.Z==zmoh_tru));
dVssed_tru = 100*diff(tm.VS(tm.Z==zsed_tru))./mean(tm.VS(tm.Z==zsed_tru));
dVsmoh_tru = 100*diff(tm.VS(tm.Z==zmoh_tru))./mean(tm.VS(tm.Z==zmoh_tru));
...
plot(gca,zmoh_tru*[1 1],axlim(4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
plot(gca,vsc1_tru*[1 1],axlim(4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
plot(gca,vsc2_tru*[1 1],axlim(4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
plot(gca,dVsmoh_tru*[1 1],axlim(gca,4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
plot(gca,tm.vpvs*[1 1],axlim(gca,4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
Ni = hist(prior.cxi,X)/prior.Npass;
bar(X,No','facecolor',[0.9 0.1 0.1],'edgecolor','none','BarWidth',1);
bar(X,Ni','facecolor','none','edgecolor',[0.2 0.2 0.2],'BarWidth',1,'LineWidth',1.5);
set(gca,'fontsize',14,'box','on','linewidth',1.5,'layer','top',...
        'xlim',[par.mod.crust.ximin,par.mod.crust.ximax],'ylim',[0 axlim(gca,4)]) 
title('Crust Xi value','fontsize',16)
plot(gca,tm.cxi*[1 1],axlim(gca,4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
Ni = hist(prior.mxi,X)/prior.Npass;
bar(X,No','facecolor',[0.9 0.1 0.1],'edgecolor','none','BarWidth',1);
bar(X,Ni','facecolor','none','edgecolor',[0.2 0.2 0.2],'BarWidth',1,'LineWidth',1.5);
set(gca,'fontsize',14,'box','on','linewidth',1.5,'layer','top',...
        'xlim',[par.mod.mantle.ximin,par.mod.mantle.ximax],'ylim',[0 axlim(gca,4)]) 
title('Mantle Xi value','fontsize',16)
plot(gca,tm.mxi*[1 1],axlim(gca,4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
...
plot(gca,Lnvg_tm*[1 1],axlim(gca,4)*[0,1],'--','linewidth',3,'color',[0.28 0.25 0.84])
