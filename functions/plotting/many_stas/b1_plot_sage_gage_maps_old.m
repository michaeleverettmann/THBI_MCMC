...
if ifig >=4; 
    ...
    label = "Vs (km/s)" + newline + sprintf('Z=%1.0f km', depths(ind_depth))
    cmap = turbo(); 
    cmap = flip(cmap); 
    colormap(gca, cmap); 
    if ind_depth == 1; 
...
cbar.Position(4) = cbar.Position(4) * .4; 
% disp(cbar.Position)
ax = gca(); 
cbar.Position(1) = (ax.Position(1)+ax.Position(3)*.85) ; 
...
m_text(-73.5, 30.75, label, 'Units','data', 'HorizontalAlignment', 'right'); 
