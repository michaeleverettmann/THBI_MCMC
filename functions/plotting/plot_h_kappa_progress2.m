...
ptbTwc = nak == 1; % brb2022.06.01 - This is the value of nonacceptk before making the perturbation. Not that comes out of delay_reject.m 
thisCond = and( ifaccept, ptbTwc); 
ac2_perc = sum(thisCond) / length(thisCond) * 100; 
ac2 = scatter(iIter(thisCond), 2+yShift+zeros(sum(thisCond),1), '|b', 'DisplayName', ...
    sprintf('Accepted 2P: %3.0f%%',ac2_perc)); 
thisCond = and(~ifaccept, ptbTwc); 
rj2_perc = sum(thisCond) / length(thisCond)* 100; 
rj2 = scatter(iIter(thisCond), 2-yShift+zeros(sum(thisCond),1), '|r', 'DisplayName', ...
    sprintf('Rejected 2P: %3.0f%%',rj2_perc)); 


% One perturbation
ptbOnce = or(nak == 0, nak == 2); 
thisCond = and( ifaccept, ptbOnce); 
ac1_perc = sum(thisCond) / length(thisCond)* 100; 
ac1 = scatter(iIter(thisCond), 1+yShift+zeros(sum(thisCond),1), '|b', 'DisplayName', ...
    sprintf('Accepted 1P: %3.0f%%',ac1_perc)); 
thisCond = and(~ifaccept, ptbOnce); 
rj1_perc = sum(thisCond) / length(thisCond)* 100; 
rj1 = scatter(iIter(thisCond), 1-yShift+zeros(sum(thisCond),1), '|r', 'DisplayName', ...
...
if par.inv.verbose; 
    ...
    exportgraphics(gcf, [resdir '/convergence_info_chain_' num2str(chainNo) '_v1.png'], 'Resolution', 100);
end
...
if par.inv.verbose; 
    ...
    exportgraphics(gcf, [resdir '/convergence_info_chain_' num2str(chainNo) '_v2.png'], 'Resolution', 100);
end
