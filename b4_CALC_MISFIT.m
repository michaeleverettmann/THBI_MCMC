...
for id = 1:length(par.inv.datatypes)
    ...
    % BODY WAVE 
    % Calculate cross-convolution misfit between observed data and (unit-normalised) predicted data
    if any(strcmp({'BW','RF'},pdt{1}))
        ...
        for itr = 1:length(trudata.(dtype))
            ...
            if strcmp(dtype, 'RF_Sp_ccp') 
                rf_tru = trudata.(dtype)(itr).PSV(:,1); 
                rf_pre = predata.(dtype)(itr).PSV(:,1);  
                zz     = trudata.(dtype).zz; 

                weight_depth_val = par.datprocess.CCP.weight_depth_val; % Upweight the deeper parts of ccp relative to shallow. First collumn: Specify depths of interest. Second collumn: Ideal weight at those depths. 
...
% % %             %%% Old option for getting misfit. Doesn't take z into account explicitly. 
