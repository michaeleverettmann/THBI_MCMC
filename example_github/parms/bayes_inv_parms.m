...
modl.starting.HKappa.startAtHK = false;             % The starting moho and vpvs will be chosen from the values that maximize the HK stack. 
...
modl.misc = struct(...
    'psi_to_xi', -1 );                              % Ratio of radial anisotropy psi to xi through whole model. Only valid if xi and psi are expressed as percent (e.g. 5%, not 1.05).  
...
modl.crust = struct(...
    ... thickness of the crust
                     ...
                     'hmin',15                   ,... %10 min xtal crust thickness, km % bb2021.10.26 Going VERY shallow because we will go offshore, expecting down to about 7 km thickness (Shuck et al., 2019). We don't want priors to thicken the offshore crust. 
...
%% Forward calc. parms
forc = struct(      'mindV',0.075                 ,... % delta Vs that will cause the layering process to make a new layer. Smaller is finer. Also scales vertical spacing of layers when there are no velocity changes. See layerise. 
...
datprocess=struct( 'normdata',true               ,... % normalise data in processing
                   ...
                   'CCP',struct(                  ... % CCP data from Emily
                      ...
                      'weight_depth_val',[-10,0.3 ; 30,0.3 ; 70,1 ; 6371,1],... % IMPORTANT This sets weighting of Sp data differencing. For example, the weighting could multiply the error by 0.3 up to 30 km depth, then up to 1.0 by 70 km depth. This reduces how much the crust dominates the results. % First collumn: Specify depths of interest. Second collumn: Ideal weight at those depths. The weights are a linear interpolation of these points which then go through a Gaussian smoothing filter. 
                      'simple_parent_pulse',true ),... ; % Needs to be true. Builds a simple Gaussian (like?) parent pulse for cross-convolution with CCP data. 
                   'HKappa',struct(              ...
                       'min_error', 0.002,       ... % Add this much "error" to h-kappa stacks (error of 0 can result in getting very low values of sigma for h-kappa, which could hamper the relative weighting between different data)
                       ...
                       'weightDistanceMax', 0,   ... % At start of burnin, gives 0 to 1 weight toward the (scaled) Euclidian distance from HKappa energy maximum. In otherwords, this tends toward disregarding the actual energy value, and pays attention to its position in h-kappa space. Not recommended. 
                       'weightDistanceMin', 0,   ... % At end of burnin, give this much weight 0 to 1 to distance from h,k where E is max.    
                       'kNum', 201,              ... % How many k values to include in the h-k grid. 
...
synth = struct( 'gcarcs',[70]                    ,... % average gcarc
                ...
                'propmat_or_telewavesim', 'propmat', ... % Whether to get receiver functions using propmat or telewavesim
...
RFparms.TB = 1.5; % period*bandwidth
RFparms.NT = 2; % number of tapers
